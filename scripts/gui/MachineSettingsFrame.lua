...
MachineSettingsFrame.XML_FILENAME = g_modDirectory .. 'data/gui/MachineSettingsFrame.xml'
...
function MachineSettingsFrame:updateMachine(vehicle)
    if vehicle == nil or vehicle == self.target.vehicle then
        ...
        local hasManagePermission = ModUtils.getPlayerHasPermission('manageRights', nil, self.target.vehicle:getOwnerFarmId())
